...
public partial class frmKhachhangTN : DevExpress.XtraEditors.XtraForm
{
    ...
    {
        if (MessageBox.Show("Xóa 1 khách hàng tiềm năng ?", "Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
        {
    ...
    private void BtnAdd_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
    {
        if (dbContext.ChangeTracker.HasChanges()) Save();
            bdSource.AddNew();
       // txtMa.Focus();
        txtMa.EditValue  = Dungchung.Sinhmadoituong("TN", 4);
        txtTen.Focus();
    }
